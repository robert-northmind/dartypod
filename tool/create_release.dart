...
Future<bool> createGitHubRelease(
  ...
  if (result.exitCode != 0) {
    stderr.writeln(
        '${Colors.red}Failed to create GitHub release: ${result.stderr}${Colors.reset}');
    return false;
  }
...
Future<void> main(List<String> args) async {
  ...
  if (!isDryRun) {
    if (!await isOnMainBranch()) {
      stderr.writeln(
          '${Colors.red}Error: Must be on main branch to create a release${Colors.reset}');
    ...
    if (!await isWorkingDirectoryClean()) {
      stderr.writeln(
          '${Colors.red}Error: Working directory has uncommitted changes${Colors.reset}');
  ...
  stdout.writeln(
      '${Colors.blue}📦 Creating release for dartypod $tag${Colors.reset}');
  stdout.writeln('');
  ...
  if (await tagExists(tag)) {
    if (isDryRun) {
      stdout.writeln(
          '${Colors.yellow}⚠ Tag $tag already exists (would fail in real run)${Colors.reset}');
      stdout.writeln('');
    } else {
      stderr.writeln(
  ...
  if (changelog.trim().isEmpty) {
    stderr.writeln(
        '${Colors.red}Error: No changelog content found for version $version${Colors.reset}');
    exit(1);
  }
  ...
  stdout.writeln(
      '  ${Colors.green}•${Colors.reset} Create git tag: ${Colors.yellow}$tag${Colors.reset}');
  stdout.writeln('  ${Colors.green}•${Colors.reset} Push tag to origin');
  stdout.writeln(
      '  ${Colors.green}•${Colors.reset} Create GitHub release with above notes');
  stdout.writeln('');
  ...
  if (!await createGitHubRelease(tag, releaseTitle, releaseNotes)) {
    stderr.writeln('');
    stderr.writeln(
        '${Colors.yellow}Tag was pushed but GitHub release creation failed.${Colors.reset}');
  ...
  stdout.writeln('  1. Go to GitHub Actions and approve the deployment');
  stdout.writeln('  2. Package will be published to pub.dev automatically');
  stdout.writeln('');
  if (repoUrl != null) {
    stdout.writeln(
