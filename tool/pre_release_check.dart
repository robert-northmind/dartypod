...
class PreReleaseChecker {
  ...
  Future<bool> _runCheck(String name, Future<bool> Function() check) async {
    ...
    try {
      ...
      stdout.writeln('${Colors.red}✗${Colors.reset} Error: $e');
      return false;
    }
  }
  ...
  Future<bool> _checkPublishDryRun() async {
    ...
    if (result.exitCode != 0) {
      stdout.writeln('');
      stdout.writeln('${Colors.yellow}Dry run output:${Colors.reset}');
  ...
  Future<bool> runPreChecks() async {
    ...
    // Summary
    stdout.writeln('');
    stdout.writeln('${Colors.blue}📊 Pre-checks Results:${Colors.reset}');
    if (_checksPassed == _checksTotal) {
      stdout.writeln('${Colors.green}✅ All $_checksTotal pre-checks passed! '
          'Ready for version bump.${Colors.reset}');
      return true;
    } else {
      stdout.writeln('${Colors.red}❌ $_checksPassed/$_checksTotal pre-checks '
  ...
  Future<bool> runPostChecks() async {
    stdout.writeln('');
    stdout.writeln(
        '${Colors.blue}🔍 Running post-version-bump checks...${Colors.reset}');
    stdout.writeln('');
    ...
    // Summary
    stdout.writeln('');
    stdout.writeln('${Colors.blue}📊 Post-checks Results:${Colors.reset}');
    if (_checksPassed == _checksTotal) {
      stdout.writeln('${Colors.green}✅ All $_checksTotal post-checks passed! '
          'Ready to push and create PR.${Colors.reset}');
      return true;
    } else {
      stdout.writeln('${Colors.red}❌ $_checksPassed/$_checksTotal post-checks '
          'passed. Please fix issues before pushing.${Colors.reset}');
      return false;
    }
  }
}
...
Future<void> main(List<String> args) async {
  ...
  if (args.isNotEmpty && args[0] == '--post') {
    ...
    stdout.writeln('');
    stdout.writeln(
        '${Colors.green}🚀 Ready to push and create PR!${Colors.reset}');
    stdout.writeln('');
    stdout.writeln('Next steps:');
    stdout.writeln('  1. Push: git push origin <branch-name>');
    stdout.writeln('  2. Create PR and merge');
    stdout.writeln(
    ...
    stdout.writeln('');
    stdout
        .writeln('${Colors.green}🚀 Ready to run version bump!${Colors.reset}');
    stdout.writeln('');
    stdout.writeln(
        'Run: ${Colors.yellow}dart tool/version_bump.dart <patch|minor|major>${Colors.reset}');
  }
}
